...
Run the normal [build workflow](../../../../tools/README.md) and `python -m unittest discover -s tools/tests -p 'test_*.py'`, then export the final Compass for the layout, observer and distance regressions. A composed preview using exported skin SVGs confirms asset selection only, not game font rendering, filters, MO2 resolution or VR behavior. The current release inputs have been verified against the local installation; the latest fixes were deployed on 2026-09-12. In-game verification is still required.

...
# 项目约定
- 根目录 AGENTS.md 的既有规则保留；按用户授权范围修改和提交；未授权时不暂存、不提交、不重置索引。此前批量提交授权已执行完毕，不自动延续到后续原型或文档任务；新提交/push须按当次授权。
...
- 显示载体变更时分别记录旧路径专用边界与可复用布局资产；独立静态原型不等于完整任务UI。未完成新路径验收前保留内嵌实现，具体迁移门槛见[掌上原型复盘](docs/palm-test-panel.md#架构转向复盘旧边界测试的价值与适用范围)。
- 文档合规检查使用 `tools/check_manager_docs.py`；当前指南与历史证据分开维护，规则变更同时核对玩家入口、专项说明、测试和双语帮助。不得用新增摘要掩盖旧指南中的冲突。

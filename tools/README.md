...
### 正式发布输入
release_files.py为build.bat、ci.py和verify_package.py提供共用规则：不带PalmTest实验资产、CNOQuestPanel或任何层级的meta.ini。链接生成的DLL构建记录必须为非实验且哈希匹配；旧DLL缺记录时先重新构建。实验资源和许可继续保留于源码树；正式包排除它们，不删除MO2管理文件。
